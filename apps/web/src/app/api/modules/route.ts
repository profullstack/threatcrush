...
export async function GET(request: NextRequest) {
  ...
  const _p = parseInt(searchParams.get("page") || "1", 10);
  const page = Number.isNaN(_p) ? 1 : Math.max(1, _p);
  const _l = parseInt(searchParams.get("limit") || "20", 10);
  const limit = Number.isNaN(_l) ? 20 : Math.min(50, Math.max(1, _l));
